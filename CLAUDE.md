...
| R3 | **Never read a whole FASTQ.** Every FASTQ touch is bounded by `--max-reads` (default 2 000; the chemistry call is N-invariant well past it, and a larger value is the explicit opt-in to read more of a full-size file) **and** `--max-bytes` (256 MB decompressed). Wall-clock is never a budget; a path that *can* stream a multi-GB FASTQ is a bug. | `PreToolUse` hook (`hooks/guards.py`, size-blind) + `test_the_read_budget_bounds_bytes_read_however_large_the_file`, `test_the_byte_budget_binds_when_the_reads_are_long` (`test_probe.py`) |
