"""Tests for ``resolve/provenance.py`` — the wrong-PDF guard (#51).

Split out of test_records.py (#113): the provenance gate is a different module from the metadata
resolver and had no mirror file of its own. The gate decides whether a staged document describes THIS
dataset at all — the natural neighbour of the resolver it guards, but a distinct rung-1 target.

The gate is a pure function over span-verified assertions + the deterministic records + the observed
winner, so most of it is tested here directly, without a workspace. A dataset-level mismatch surfaces
an open Conflict (a human decides); the strongest case — nothing matches AND the assay contradicts the
reads — refuses with a Blocker. The final test drives the whole `_fill_manifest_pipeline`, proving the
gate's wiring (records -> gate -> validate) end to end.
"""
