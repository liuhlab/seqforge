...
def eval_run(
    ...
    `deepseek-v4-pro` on the DeepSeek preset, and this corpus is what settled it: run head-to-head
    at `ac11b44`, pro was 2.6x faster and spent 3.5x fewer output tokens than `deepseek-v4-flash`,
    so the cheap-end argument for flash lost on its own terms (#188). The report names whichever
    ran, since the numbers are a claim about that extractor and not about the harness.
    ...
    if coverage.get("cases_unmeasured") or coverage.get("cases_partial"):
        ...
        # Name the model that ran, and never prescribe one. This line used to advise
        # `--model deepseek-v4-pro` unconditionally, which it also printed on a run of pro — a
        # remedy that does not know what it is remedying, and the reason `extractor` is resolved
        # rather than echoed: the effective model is knowable here even when `--model` was unset.
        ...
        typer.echo(
            ...
            f"narrower than it looks — re-run those cases with a smaller --jobs, or with a --model "
            f"other than {ran}",
            err=True,
        )
