...
from ..workflows import WorkflowModule, get_module
from .params import (
    derived_params,
    find_read_with_role,
    param_block_key,
...
#: no arguments do the right thing. This is also what PROJECT_BRIEF §8 specified all along.
...
#
# Run it:  snakemake --profile <your-cluster-profile>
# The default target is `all`, which produces one count matrix per sample in units.tsv.
...
def compose(
    ...
    *,
    ...
    p = plan(manifest, processing, registry=registry, outdir=outdir, fastq_dir=fastq_dir)
