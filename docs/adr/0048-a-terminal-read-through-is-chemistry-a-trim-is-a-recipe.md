...
does not, and is declared in the entry as `read_through`. The price, accepted, is a `kb_version`
bump that re-keys `run_id`: a compiled dataset gets a new pipeline directory and its CRAMs are not
reused, which is correct because the reads really are processed differently.
