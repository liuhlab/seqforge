...
does not, and is declared in the entry as `read_through`. The price, accepted, is that `read_through`
is one of the fields `run_id` hashes (ADR-0037), so declaring one re-keys that chemistry's compiled
datasets: each gets a new pipeline directory and does not reuse its CRAMs, which is correct because
the reads really are processed differently.
