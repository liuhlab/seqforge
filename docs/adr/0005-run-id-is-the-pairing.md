...
`run_id = H(dataset_hash ⊕ processing_hash ⊕ spec_hash ⊕ workflow_version)`, recorded in the
...
**Status.** Supersedes `provenance_id(manifest_hash, kb_version, workflow_version)`. Amended — the
third component narrowed from a repository-wide version to the deciding spec's content hash.
